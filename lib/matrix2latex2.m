...
% Enjoy life!!!
    ...
    okargs = {'rowlabels','mrow','columnlabels', 'alignment', 'format', 'size'};
    for j=1:2:(nargin-2)
        pname = varargin{j};
        pval = varargin{j+1};
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 2  % rowlabels
                    rowLabels2 = pval;
                    if isnumeric(rowLabels2)
                        rowLabels2 = cellstr(num2str(rowLabels2(:)));
                    end                    
                case 3  % column labels
                    colLabels = pval;
    ...
    m = ceil(length(rowLabels)/length(rowLabels2));
    ...
    if(~isempty(colLabels))
        ...
        for w=1:width-1
            fprintf(fid, '\\textrm{%s}&', colLabels{w});
        end
        fprintf(fid, '\\textrm{%s}\\\\\r\n', colLabels{width});
    end
    ...
    it = 0;
    for h=1:height
        if (~isempty(rowLabels2) && mod(h, m) == 1)
            it = it + 1;
            fprintf(fid, '\\hline');
            fprintf(fid, '\\multirow{%s}{*}{%s}&', num2str(m), rowLabels2{it});
        else
            fprintf(fid, '&');
        end
        if(~isempty(rowLabels))
            fprintf(fid, '\\textrm{%s}&', rowLabels{h});
        end
        for w=1:width-1
